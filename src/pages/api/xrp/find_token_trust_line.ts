...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseData>) {
  try {
    ...
    const lines = await WEB3.getTokenTrustLine(network === 1, chainId, String(address))
    if (!lines) {
      return res.status(200).json({ message: 'Cannot find trust line', result: false, data: null })
    }

    return res.status(200).json({ message: '', result: true, data: lines })
  } catch (e) {
    console.error(e)
