...
export class ETH {
  ...
  static async getTransactionDetail(
    ...
    try {
      if (isPending) {
        ...
        const params = [hash]
        const tx_response = await RPC.callRPC(
          this.getChainIds(isMainnet),
          TRANSACTIONFUNCS.GETTXBYHASH,
          params
        )
        ...
        if (token) {
          const {
            ...
            tokenAmount,
            token: COINS,
          } = await this.getTokenTransferToAmountAndTokenByInput(
            isMainnet,
            tx_response.result.input
          )
        ...
        const receipt_response = await RPC.callRPC(
          this.getChainIds(isMainnet),
          TRANSACTIONFUNCS.GETTXRECEIPT,
          params
        )
  ...
  static async sendAccelerateTransaction(
    isMainnet: boolean,
    request: CreateEthereumTransaction
  ): Promise<string> {
    ...
    if (request.maxPriorityFeePerGas) {
      request.maxPriorityFeePerGas = new Big(request.maxPriorityFeePerGas)
        .mul(150)
        .div(100)
        .toString()
    } else {
