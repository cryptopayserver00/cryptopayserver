...
import { SolanaWalletType } from '@/utils/types'

const Solana = () => {
  const [isSettings, setIsSettings] = useState<boolean>(false)
  const [wallet, setWallet] = useState<SolanaWalletType[]>([])
      ...
      <div className="mx-auto max-w-screen-xl px-4">
        <div className="flex flex-wrap items-center justify-between gap-4 pt-10">
          <div className="flex items-center gap-2">
            <Image src={SolanaSVG} alt="solana" width={50} height={50} className="h-12 w-12" />
        ...
        <div className="mt-10">
          <Card className="inline-block">
            <CardContent className="px-10 py-6 text-center">
              <p className="text-muted-foreground">Fee Per Signature</p>
              <p className="mt-2 font-semibold">{feeRate} Sol</p>
            </CardContent>
          </Card>
        </div>
          ...
          {rows && rows.length > 0 ? (
            rows.map((row, index) => (
              <TableRow key={index}>
                <TableCell>
                  <Link
                    href={GetBlockchainTxUrl(getNetwork() === 'mainnet', row.hash)}
                    target="_blank"
                ...
                <TableCell>
                  {new Date((row.blockTimestamp as number) * 1000).toLocaleString()}
                </TableCell>
